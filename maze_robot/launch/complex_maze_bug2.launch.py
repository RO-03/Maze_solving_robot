...
============================
Pure Bug2 baseline on the complex maze (no frontier exploration).
The goal is a fixed waypoint set via parameter.
No frontier_node launched – this is a point-to-point navigation run.
"""
...
def generate_launch_description():
    ...
    spawn = TimerAction(period=5.0, actions=[
        Node(
            ...
            arguments=[
                ...
                '-name', 'maze_robot', '-world', 'maze_complex', '-z', '0.2',
            ],
        )
    ])
    ...
    slam = Node(
        ...
        parameters=[{
            ...
            'mode': 'mapping', 'resolution': 0.05, 'max_laser_range': 10.0,
    ...
    algo = Node(
        package=pkg_name, executable='bug2_baseline_node', output='screen',
        parameters=[{
            'use_sim_time': True,
            'goal_x': 0.0, 'goal_y': 8.0,
            'map_frame': 'map', 'robot_frame': 'base_footprint', 'use_tf': True,
            'max_linear_speed': 0.28, 'max_angular_speed': 1.2,
            'kp_angular': 1.2, 'goal_radius': 0.4,
