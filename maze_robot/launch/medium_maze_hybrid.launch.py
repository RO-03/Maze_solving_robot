...
def generate_launch_description():
    ...
    sensor   = Node(package=pkg_name, executable='sensor_node',           output='screen',
                    parameters=[{'use_sim_time': True, 'noise_std_dev': 0.02,
                                 ...
                                 'publish_rate': 20.0}])
    frontier = Node(package=pkg_name, executable='frontier_node',         output='screen',
                    parameters=[{'use_sim_time': True, 'goal_publish_rate': 1.0,
                                 'min_frontier_size': 6, 'goal_arrival_radius': 0.45,
                                 'map_frame': 'map', 'robot_frame': 'base_footprint',
                                 'use_tf': True}])
    algo     = Node(package=pkg_name, executable='hybrid_algorithm_node', output='screen',
                    parameters=[{'use_sim_time': True, 'map_frame': 'map',
                                 'robot_frame': 'base_footprint', 'use_tf': True,
